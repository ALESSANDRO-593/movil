...
// Puente entre Angular/Ionic y el plugin nativo Android que creamos
// en RecaptchaPlugin.java.
interface NativeRecaptchaPlugin {
  initialize(): Promise<{ ready: boolean }>;
  execute(options: { action?: string }): Promise<{ token: string }>;
}

const NativeRecaptcha =
  registerPlugin<NativeRecaptchaPlugin>('NativeRecaptcha');
...
const OPCIONES_MENU_ESTUDIANTE: OpcionChat[] = [
  {
    id: 'CERTIFICADO_MATRICULA',
    etiqueta: 'Solicitar Certificado de Matrícula',
    icono: 'document-text-outline',
    disponible: true
  },
  {
    id: 'ANULACION_MATRICULA',
    etiqueta: 'Solicitar Anulación de Matrícula',
    icono: 'close-circle-outline',
    disponible: true
  },
  {
    id: 'RESET_CORREO',
    etiqueta: 'Resetear contraseña de correo institucional',
    icono: 'key-outline',
    disponible: true
  },
  {
    id: 'FINALIZAR_CONVERSACION',
    etiqueta: 'Finalizar conversación',
    icono: 'exit-outline',
    disponible: true
  }
];
...
const OPCIONES_MENU_DOCENTE: OpcionChat[] = [
  {
    id: 'REPORTAR_INCIDENCIA_LAB',
    etiqueta: 'Reportar incidencia en laboratorio',
...
export class ChatPage implements OnInit, OnDestroy {
  ...
  // Detecta si YaviBot está corriendo dentro de la APK Android.
  readonly esAndroid = Capacitor.getPlatform() === 'android';

  // Indica si el SDK nativo Android de reCAPTCHA ya está inicializado.
  private nativeRecaptchaReady = false;
  ...
  ngOnInit(): void {
    ...
    // APK Android → reCAPTCHA nativo.
    // Navegador → reCAPTCHA web que ya teníamos funcionando.
    if (this.esAndroid) {
      void this.inicializarCaptchaAndroid();
    } else {
      this.renderCaptcha();
    }
  ...
  private async iniciarMonitorDeRed(): Promise<void> {
    ...
    this.listenerRed = await Network.addListener(
      'networkStatusChange',
      estado => {
        this.ngZone.run(() => {
          this.sinConexion = !estado.connected;
        });
      }
    );
  }
  ...
  /**
   * Inicializa reCAPTCHA nativo cuando la aplicación corre como APK Android.
   *
   * La clave Android está configurada directamente dentro de
   * RecaptchaPlugin.java, así que Angular no necesita conocerla.
   */
  private async inicializarCaptchaAndroid(): Promise<void> {
    if (!this.esAndroid || this.nativeRecaptchaReady) {
      return;
    }

    try {
      const resultado = await NativeRecaptcha.initialize();
      this.nativeRecaptchaReady = resultado.ready;
    } catch (error) {
      this.nativeRecaptchaReady = false;
      console.error(
  ...
  /**
   * Genera un token nuevo desde el SDK nativo Android.
   *
   * Este token se pide justo antes de consultar la cédula, por lo que
   * Android no necesita mostrar la casilla visual "No soy un robot".
   */
  private async obtenerCaptchaAndroid(): Promise<string | null> {
    try {
      if (!this.nativeRecaptchaReady) {
        await this.inicializarCaptchaAndroid();
      }
      ...
      const resultado = await NativeRecaptcha.execute({
      ...
      console.error(
        'No se pudo obtener token reCAPTCHA Android:',
        error
      );
  ...
  /**
   * Renderiza el widget de reCAPTCHA únicamente en la versión WEB.
   *
   * En Android no se ejecuta este método porque la APK utiliza el
   * plugin nativo NativeRecaptcha.
   */
  private renderCaptcha(intentos = 0): void {
    ...
    const contenedor =
      document.getElementById('recaptcha-cedula');
   ...
   *
   * WEB:
   *   limpia el token y reinicia visualmente el checkbox.
   *
   * ANDROID:
   *   limpia el token; la siguiente consulta solicita uno nuevo al SDK.
   */
  ...
  get opcionesMenu(): OpcionChat[] {
    return this.usuarioActual?.tipoUsuario === 'DOCENTE'
      ? OPCIONES_MENU_DOCENTE
  ...
  private async iniciarConversacion(): Promise<void> {
    ...
    await this.hablar(
      '¡Hola! 👋 Soy YaviBot, tu asistente virtual académico.'
    );

    await this.hablar(
      'Para ayudarte, primero necesito verificar tu identidad. Por favor ingresa tu número de cédula.'
    );
  ...
  /**
   * En web necesitamos cédula válida + checkbox reCAPTCHA.
   *
   * En Android solamente necesitamos que la cédula sea válida:
   * el token nativo se genera automáticamente al presionar Enviar.
   */
  get puedeEnviarCedula(): boolean {
    const cedulaValida =
      validarCedulaEcuatoriana(this.cedulaIngresada);

    const captchaListo =
      this.esAndroid || !!this.captchaToken;
  ...
  async enviarCedula(): Promise<void> {
    ...
    // El checkbox solamente existe en la versión web.
    if (!this.esAndroid && !this.captchaToken) {
      this.errorCedula =
    ...
    /**
     * Si estamos dentro de la APK Android, solicitamos aquí el token
     * al plugin nativo antes de llamar al workflow de n8n.
     */
    if (this.esAndroid) {
      captchaToken =
        await this.obtenerCaptchaAndroid();
    ...
    this.estudianteService
      ...
      .subscribe({
        next: async usuario => {
          ...
          if (!usuario) {
            await this.hablar(
              'No encontré ningún estudiante ni docente con esa cédula. ¿Puedes verificar el número e intentarlo de nuevo?'
            );
          ...
          await this.enviarTicketDeVerificacion(
            usuario
          );
        },
        ...
        error: async (error: HttpErrorResponse) => {
          ...
          console.error('ERROR CONSULTAR CEDULA:', error);

          await this.hablar(
            `Error consultando datos. HTTP ${error.status || 0} - ${
              error.error?.message ||
              error.error?.error ||
              error.message ||
              'Error desconocido'
  ...
  private async enviarTicketDeVerificacion(
    usuario: Usuario
  ): Promise<void> {
    ...
    this.estudianteService
      .enviarTicketVerificacion(usuario.cedula)
      .subscribe({
        next: async resultado => {
          ...
          if (resultado.ticketDebugSoloMock) {
            await this.hablar(
              `🔧 Modo prueba: tu ticket es ${resultado.ticketDebugSoloMock} (esto desaparecerá cuando el backend real envíe el correo).`
            );
          }
        ...
        error: async () => {
          ...
          await this.hablar(
            'No pude generar el ticket de verificación. Intenta nuevamente en unos segundos.'
          );
  ...
  async verificarTicketIngresado(): Promise<void> {
    if (
      !this.puedeVerificarTicket ||
      !this.usuarioActual
    ) {
      this.errorTicket =
        `Ingresa el ticket de ${LONGITUD_TICKET_VERIFICACION} dígitos que enviamos a tu correo.`;
    ...
    this.estudianteService
      ...
      .subscribe({
        next: async valido => {
          ...
          if (!valido) {
            await this.hablar(
              'Ese ticket no es correcto. Verifica tu correo e inténtalo de nuevo.'
            );
          ...
          await this.hablar(
            `¡Identidad verificada! ✅ Hola ${this.nombreVisible(usuario)}, ¿qué deseas consultar hoy?`
          );
        ...
        error: async (
          ...
          if (error.status === 429) {
            await this.hablar(
              'Hiciste demasiados intentos con ese código. Por seguridad, vamos a solicitar uno nuevo.'
            );
          ...
          await this.hablar(
            'Tuve un problema verificando el ticket. Intenta nuevamente en unos segundos.'
          );
    ...
    this.estudianteService
      ...
      .subscribe({
        next: async resultado => {
          ...
          await this.hablar(
            `Te reenviamos un nuevo código a tu correo institucional (${resultado.correoEnmascarado}).`
          );

          if (resultado.ticketDebugSoloMock) {
            await this.hablar(
              `🔧 Modo prueba: tu ticket es ${resultado.ticketDebugSoloMock}.`
            );
          }
        },
        ...
        error: async (
          ...
          if (error.status === 429) {
            await this.hablar(
              'Ya te enviamos un código hace unos segundos. Espera un momento antes de pedir otro.'
            );
          } else {
            await this.hablar(
              'No pude reenviar el código en este momento. Intenta nuevamente en unos segundos.'
            );
          }
        }
      });
  ...
  async seleccionarOpcion(
    ...
    if (!opcion.disponible) {
      await this.hablar(
        'Esa opción todavía está en construcción 🚧, muy pronto estará disponible.'
      );
    ...
    if (
      ...
      this.esEstudiante(usuario)
    ) {
      await this.hablar(
        'Estos son tus datos. Verifícalos antes de generar tu certificado:'
      );

      this.agregarMensaje({
        tipo: 'bot-preview-certificado',
        estudiante: usuario
      });
    ...
    if (
      ...
      await this.iniciarAnulacionMatricula(
        usuario
      );
    ...
    if (
      opcion.id === 'RESET_CORREO' &&
      this.esEstudiante(usuario)
    ) {
      await this.iniciarReseteoCorreo(
        usuario
      );
  ...
  private nombreVisible(
    ...
    return this.esEstudiante(usuario)
      ? `${usuario.nombres} ${usuario.apellidos}`.trim()
      : usuario.nombres;
  }

  private async iniciarAnulacionMatricula(
    estudiante: Estudiante
  ): Promise<void> {
    await this.hablar(
    ...
    await this.hablar(
      'Por favor confirma los datos de tu solicitud antes de continuar:'
    );

    this.agregarMensaje({
      tipo: 'bot-confirmacion-anulacion',
      estudiante
    });
    ...
    this.estudianteService
      ...
      .subscribe({
        next: async ticket => {
          ...
          await this.hablar(
            `✅ Tu solicitud de anulación ha sido registrada exitosamente con el número de ticket ${ticket.id}.`
          );
        ...
        error: async (
          ...
          await this.hablar(
            'No pude registrar tu solicitud en este momento. Intenta nuevamente en unos segundos, o elige otra opción.'
          );
  ...
  async cancelarAnulacion(): Promise<void> {
    ...
    await this.hablar(
      'Solicitud cancelada. ¿En qué más puedo ayudarte?'
    );
  ...
  private async iniciarReseteoCorreo(
    estudiante: Estudiante
  ): Promise<void> {
    await this.hablar(
    ...
    this.estudianteService
      ...
      .subscribe({
        ...
        error: async (
          ...
          await this.hablar(
            'No pude completar el reseteo en este momento. Intenta nuevamente en unos segundos, o elige otra opción.'
          );
  ...
  private async finalizarConversacion(): Promise<void> {
    await this.hablar(
      '¡Gracias por usar YaviBot! Que tengas un excelente día 👋'
    );
    ...
    this.estudianteService
      ...
      .subscribe({
        next: async certificado => {
          ...
          await this.hablar(
            '¡Listo! Tu certificado de matrícula ha sido generado exitosamente. 🎓'
          );
          ...
          try {
            ...
            await firstValueFrom(
              this.estudianteService.enviarCertificadoPdf(
                certificado.cedula,
            ...
            await this.hablar(
              '⚠️ Generé tu certificado, pero no pude enviarlo por correo en este momento. Puedes intentarlo nuevamente más tarde.'
            );
          }

          await this.hablar(
            'Este QR es único e irrepetible: podrá verificarse en el sistema web con este mismo código. ¿Deseas algo más?'
          );
        ...
        error: async (
          ...
          if (error.status === 400) {
            await this.hablar(
              error.error?.error ??
              'No puedes generar el certificado: no apareces matriculado en el periodo actual.'
            );
          } else {
            await this.hablar(
              'No pude generar el certificado en este momento. Intenta nuevamente en unos segundos, o elige otra opción.'
            );
          }
    ...
    this.estudianteService
      ...
      .subscribe({
        next: async laboratorios => {
          ...
          await this.hablar(
            '¿En qué laboratorio ocurrió la incidencia?'
          );

          this.agregarMensaje({
            tipo: 'bot-laboratorios',
            laboratorios
          });
        },
        ...
        error: async () => {
          ...
          await this.hablar(
            'No pude cargar la lista de laboratorios. Intenta nuevamente en unos segundos.'
          );
  ...
  async seleccionarLaboratorio(
    ...
    await this.hablar(
      'Describe brevemente qué ocurrió (equipo afectado, hora aproximada, etc.):'
    );
  ...
  async enviarDescripcionIncidencia(): Promise<void> {
    if (
      !this.puedeEnviarDescripcionIncidencia ||
      !this.laboratorioSeleccionado
    ) {
      return;
    }
    ...
    await this.hablar(
      '¿Quieres adjuntar una foto de la incidencia? Es opcional.'
    );
  ...
  async onFotoSeleccionada(
    ...
    if (
      ...
      this.errorFoto =
        'Solo se aceptan fotos en formato JPG o PNG.';
    ...
    if (
      ...
      this.errorFoto =
        'La foto no puede pesar más de 5MB.';
    ...
    try {
      ...
      this.errorFoto =
        'No se pudo leer la foto. Intenta con otra o continúa sin foto.';
    ...
    this.fotoSeleccionada = {
      ...
      mime: archivo.type,
      previewUrl:
        `data:${archivo.type};base64,${base64}`
    };

    this.agregarMensaje({
      tipo: 'usuario-texto',
      texto: '📷 Foto adjuntada'
    });
  ...
  private async mostrarConfirmacionIncidencia(): Promise<void> {
    ...
    this.agregarMensaje({
      ...
      descripcionIncidencia:
        this.descripcionPendiente,
      fotoPreviewUrl:
        this.fotoSeleccionada?.previewUrl
    });
  ...
  async confirmarReporteIncidencia(): Promise<void> {
    ...
    const foto =
      this.fotoSeleccionada
        ? {
            base64:
              this.fotoSeleccionada.base64,
            mime:
              this.fotoSeleccionada.mime
          }
        : undefined;
    ...
    this.estudianteService
      .reportarIncidenciaLaboratorio(
        this.usuarioActual.cedula,
        this.laboratorioSeleccionado.codigo,
        this.descripcionPendiente,
        foto
      )
      .subscribe({
        next: async incidencia => {
          ...
          await this.hablar(
            'El equipo de laboratorios revisará la incidencia. ¿Deseas hacer algo más?'
          );
        ...
        error: async (
          ...
          await this.hablar(
            'No pude registrar tu reporte en este momento. Intenta nuevamente en unos segundos, o elige otra opción.'
          );
  ...
  async cancelarReporteIncidencia(): Promise<void> {
    ...
    await this.hablar(
      'Reporte cancelado. ¿En qué más puedo ayudarte?'
    );
  ...
  private async manejarSesionExpirada(): Promise<void> {
    await this.hablar(
      '🔒 Por seguridad, tu verificación anterior ya expiró. Vamos a comenzar de nuevo para confirmar tu identidad.'
    );
  ...
  private async requiereConexion(): Promise<boolean> {
    ...
    await this.hablar(
      '📡 No tienes conexión a internet en este momento. Verifica tu conexión e inténtalo de nuevo.'
    );
  ...
  private async hablar(
    texto: string,
    tiempoEscribiendo = 700
  ): Promise<void> {
    this.mostrarEscribiendo();
